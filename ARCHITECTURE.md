...
1. **Phase 1 — place group-winners (`group_pos == top`)**, using `bye_hierarchy(num_slots)` (the classic power-of-two seeded-slot subdivision order). Batch 0 (slot 1) and the last batch (last slot) are placed deterministically (seed #1, #2); later batches use the shared **`place_bye_greedy`** helper: greedy scoring (`score_candidate_placement`, via `score_bracket`) plus heavy penalties for unbalancing top-seed density across quarters/halves. The half-level penalty balances the **net load `tops − byes`** per half, not the raw top count: because a group winner forces its 2nd/3rd (delta 1/2) into the *opposite* half, the demand on a half equals the winners opposite it, while a bye consumes a slot with no opposite demand — so each bye a half holds requires that half to carry one extra winner to stay feasible (byes and winners co-locate). Balancing raw winners alone treated a 3-2 and a 2-3 half split as equal even when one is un-fillable given where the bye sits, which forced needless `quarter_capacity_degrade` fallbacks (see [§9](#9-known-issues)). Each group's winner slot is recorded as the "anchor" (`_update_group_top`) for later rules. **Returns early here if `max_draw_phase <= 1`.**
...
4. **Phase 4** — groups remaining (non-bye, non-winner) players into per-quarter candidate pools per their Phase-2 assignment; a `"quarter_pool_size_mismatch"` here is a defensive guard (Phase 2's capacity check makes it unreachable in normal flow).
...
- **Phase 1 half-balance ignores per-group opposite-load asymmetry** (open — fixes S W1 main but not consolation): the `tops − byes` half penalty (see [§5](#5-algorithms-draw) Phase 1) assumes every group winner generates the same opposite-half demand (2 players: 2nd + 3rd). That holds when every group contributes the same positions — e.g. **S W1 main** (5 groups × pos 1/2/3), which the fix makes degrade-free. It does **not** hold when groups are uneven, e.g. **S W1 consolation** (14 players: g1–4 give pos 4/5/6, but g5 only pos 4/5). There g5's winner demands *one* opposite-half player, not two, so counting it as a full `+1` mis-balances the halves and can still route the draw through the `quarter_capacity_degrade` fallback (the output is typically still violation-free, just via the soft path). The correct generalization is to weight each winner by its group's actual opposite-load minus same-half-load (`weight(g) = 1 + d3_g − d12_g`, which collapses to the current `tops − byes` when every `d12_g == 2, d3_g == 0`) and balance that weighted sum across halves. Not yet implemented.
