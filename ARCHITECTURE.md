...
| | `half_split_weight`, `first_vs_first_weight`, `country_half_weight`, `base_first_weight` | ⚠️ Defined but currently unused — see [§9](#9-known-issues). |
...
- **`find_players_not_in_draw_data` doesn't do what its docstring says** — `checks/validity_checker.py:30-42`: its docstring says "players in `players_by_start_number` but not referenced in `draw_data`" (i.e. `all_players - referenced_players`), but the body computes `referenced_players - all_players` — identical logic to `find_missing_players` above it. It currently returns the same (typically empty, since `find_missing_players` already validated no missing refs) result rather than the intended "imported but never entered in any competition" set.
- **Gender/competition mismatch check no longer aborts** — `misc/initializer.py:139`: the `#return` after printing `find_players_in_wrong_competition` errors is commented out, so this check is now warn-only despite the surrounding code structure (and `spinner.fail()`) suggesting it should halt the run.
- **Unreachable `'Back'` case in the S/D/M menu** — `misc/menu.py:38-61`: `singles_doubles_mixed_choice()` has a `case 'Back': view_choice()` branch, but the `choices` list passed to `inquirer.list_input` (line 40) is `['Singles', 'Doubles', 'Mixed']` — `'Back'` is never offered, so that case can never trigger.
- **`initialize_config` declares but never sets `global mode`** — `misc/config.py:9-11`: `mode` is declared global but never assigned from `config["settings"]["mode"]`; the interactive/normal mode is read directly from `config["settings"]["mode"]` wherever needed (e.g. `viewer/bracket_viewer.py`), making this global dead.
- **`[bracket_draw]` scoring weights in `config.ini` are not wired up** — `checks/bracket_checker.py`'s `score_bracket(matches, number_of_matches, weights=None)` is called from `draw/bracket_drawer.py` at every call site (lines 188, 210, 228, 306, 549, 659, 674, 745, 796, 817, 863, 901) with no `weights` argument, so it always falls back to its own hardcoded defaults (`quarter_split=200, half_split=150, first_vs_first=100, country_half=10, base_first=20`). The `half_split_weight`, `first_vs_first_weight`, `country_half_weight`, `base_first_weight` keys in `config.ini`'s `[bracket_draw]` section currently have no effect on the algorithm, and there's no `quarter_split_weight` key in config at all despite it being the heaviest-weighted term.
...
- **`tests/run_bracket_smoke.py` checks against stale snapshot action names** — its anchoring-regression check includes `'half0_mc_start'`/`'half1_mc_start'` in its expected action-name list, which the current `bracket_drawer.py` no longer emits (it now only emits `quarter{0-3}_mc_start`). Harmless (just unused list entries) but a leftover from a prior half-based (rather than quarter-based) implementation.
...
- **`random_seed` doesn't make all randomness deterministic** — `misc/config.py:22-23` seeds the module-global `random` module (`random.seed(random_seed)`, passed as a raw string) once at startup; this is what `draw/group_drawer.py`'s Monte Carlo loop uses. But `draw/bracket_drawer.py` creates its own separate `random.Random()` instance, seeded from `int(config["settings"]["random_seed"])` (a different value representation, and a different RNG stream entirely). Both are nominally "seeded" by the same config value but do not produce coordinated/reproducible interaction between group and bracket draws as a whole — this was flagged as a known issue in the original README ("Make seed work along all random functionalities").
