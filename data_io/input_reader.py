...
def read_draw_data() -> list[DrawDataRow]:
    ...
    with open(draw_data_file_path, "r", encoding="utf-8") as file:
        ...
        draw_data = []
        for line in lines[1:]:
            competition, competition_class, amount_of_groups, seeding, group_no, group_pos, main_round, consolation_round, start_number_a, start_number_b = [field.strip() for field in line.strip().split(";")]
...
def read_players() -> list[Player]:
    ...
    with open(player_file_path, "r", encoding="utf-8") as file:
        ...
        players = []
        for line in lines[1:]:
            start_number, last_name, first_name, country, base, gender, qttr = [field.strip() for field in line.strip().split(";")]
